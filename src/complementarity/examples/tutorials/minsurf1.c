...
/* 
   Include "tao.h" so we can use the TAO solvers with PETSc support.
*/
static char  help[] =
...
/*T
   ...
   Routines: TaoSetConstraintRoutine(); TaoSetJacobianRoutine();
   Routines: TaoGetVariableBounds(); TaoAppSetInitialSolutionVec();
...
int main(int argc, char **argv)
{
  ...
  PetscScalar lb = -TAO_INFINITY;      /* lower bound constant */
  ...
  info = TaoCreate(PETSC_COMM_SELF, "tao_ssils", &tao); CHKERRQ(info);
  info = TaoApplicationCreate(PETSC_COMM_SELF, &my_app); CHKERRQ(info);
  ...
  info = TaoSetupApplicationSolver(my_app,tao); CHKERRQ(info);
