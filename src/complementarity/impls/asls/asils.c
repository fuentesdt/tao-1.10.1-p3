...
static int TaoSolve_ASILS(TAO_SOLVER tao, void *solver)
{
  ...
  while (1) {
    ...
    info = PetscPrintf(((PetscObject)tao)->comm,"Fixed size: %d\n", nf); CHKERRQ(info);
    ...
    if (innerd <= delta*pow(normd, rho)) {
       printf("Gradient direction: %5.4e.\n", innerd);
       info = PetscInfo1(tao, "TaoSolve_ASILS: %d: newton direction not descent\n", iter); CHKERRQ(info);
...
int TaoCreate_ASILS(TAO_SOLVER tao)
{
  TAO_SSLS *asls;
  int        info;
  ...
  info=TaoSetTaoSolveRoutine(tao,TaoSolve_ASILS,(void*)asls); CHKERRQ(info);
